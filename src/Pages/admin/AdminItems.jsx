...
const AdminItems = () => {
  ...
  const fetchItems = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/admin/items?${params}`, {
  ...
  const handleBanItem = async () => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const res = await fetch(`http://localhost:5000/admin/items/${itemToBan}/ban`, {
        method: 'PUT',
        headers: {
  ...
  const handleUnbanItem = async (itemId) => {
    try {
      const token = localStorage.getItem('adminToken');
      const res = await fetch(`http://localhost:5000/admin/items/${itemId}/unban`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDeleteItem = async (itemId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const res = await fetch(`http://localhost:5000/admin/items/${itemId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
